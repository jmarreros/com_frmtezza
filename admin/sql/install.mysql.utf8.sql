...
CREATE OR REPLACE VIEW `#__frmtezza_v_user_forms` AS SELECT tfu.id, fr.title, u.id id_user, tfu.id_record id_record, fr.name frmname, u.name, ug.title as area, ug.id as id_area, tfu.dt_register, tfu.approval, ub.name boss, tfu.observation, tfu.dt_approval, urh.name boss_rrhh, tfu.approval_rrhh, tfu.observation_rrhh, tfu.dt_approval_rrhh
