...
CREATE OR REPLACE VIEW `#__frmtezza_v_user_forms` AS SELECT tfu.id, tfu.id_record, fr.title, fr.name as frmname, u.id id_user, u.name, ug.title as area, ug.id as id_area, tfu.dt_register, tfu.approval, ub.name boss, tfu.observation, tfu.dt_approval, urh.name boss_rrhh, tfu.approval_rrhh, tfu.observation_rrhh, tfu.dt_approval_rrhh
...
LEFT JOIN `#__users` AS ub ON ub.id = tfu.id_boss
LEFT JOIN `#__users` AS urh ON urh.id = tfu.id_boss_rrhh;
